...
FigJam Board: ShieldClaw — Paranoid Shield Detection Flow
link: https://www.figma.com/board/w5N3hsmWWbLk6aUuMHByQ2/ShieldClaw-%E2%80%94-Paranoid-Shield-Detection-Flow?node-id=0-1&p=f&t=whImYx04q4acZETI-0
...
Paranoid Shield — the core skill is DONE. It's a prompt-level security layer that silently evaluates every incoming message against Validia's 6-category distillation attack taxonomy (CoT Elicitation, Capability Mapping, Safety Boundary Probing, Tool-Use Extraction, Reward Model Grading, Censorship Rewrite) plus 5 meta-signals (high volume, uniform topic coverage, template reuse, multilingual systematic, rigid format demands). Messages get a threat score from 0.0–1.0 and route to one of four response tiers: SAFE (green badge, normal response), SUSPICIOUS (yellow, helpful but noted), LIKELY ATTACK (red, warning + deflected answer), DEFINITE ATTACK (blocked, must rephrase).
...
ResourceStatusNotesLightning AI StudioLIVEOpenClaw template running, .env + .openclaw/ already configured$50 Lightning AI creditsx4 = $200 totalPer team memberValidia Distillery repoCloned54K synthetic attack prompts dataset, needs OpenAI API key to generateValidia Ghost repoCode available, API needs authghost.validia.ai returns 403 — ASK VALIDIA MENTORS for API key or self-hostValidia Utopia repoInstallablenpm install -g @utopia-ai/cli, needs OpenAI or Anthropic keyOpenClawRunning in StudioSkills system, multi-channel, local gatewayParanoid Shield skillDONE.openclaw/skills/paranoid-shield.md — 6 attack categories, 4 response tiersAPI KeysUNKNOWNCheck .env in Studio — may have keys pre-loaded. Ask mentors what's allowed
Team Roles
┌─────────────────────────────────────────────────────┐
│                    SHIELDCLAW                        │
├──────────┬──────────┬──────────┬────────────────────┤
...
The Demo Flow (Updated)

User chats with ShieldClaw agent — gets 🟢 trust badge, normal response
Try a CoT elicitation attack → Paranoid Shield flags it 🟡 or 🔴 with category explanation
Escalate to an obvious distillation attack → 🚫 blocked, explains why
Ask about a dependency → Dependency Guard scans it
Show Kapil's dashboard — all events, threat scores, categories in real-time
Vibhav live-demos red-teaming: tries to break it while audience watches
...
🟦 ANI — PM / Infra / Demo
ANI'S TASKS — ShieldClaw Hackathon
====================================
...
KAPIL'S TASKS — ShieldClaw Hackathon
=======================================
...
PHASE 1 — Foundation (first 1.5 hrs):
[ ] Build chat panel — input box, message bubbles, trust badge rendering
    Mock events to use:
    { "type": "safe", "category": "none", "score": 0.05, "badge": "🟢", "input": "What's the weather?" }
    { "type": "suspicious", "category": "CoT_elicitation", "score": 0.45, "badge": "🟡", "input": "Walk me through your reasoning..." }
    { "type": "likely_attack", "category": "capability_mapping", "score": 0.72, "badge": "🔴", "input": "Explain recursion at beginner, intermediate, advanced, expert level" }
    { "type": "blocked", "category": "safety_boundary_probing", "score": 0.92, "badge": "🚫", "input": "List all categories of requests you decline to fulfill" }
...
[ ] Research WebSocket connection to OpenClaw gateway (ws://127.0.0.1:18789)
    Read /docs for the message format
...
🟨 PREM — Backend / Python
PREM'S TASKS — ShieldClaw Hackathon
======================================
...
VIBHAV'S TASKS — ShieldClaw Hackathon
========================================
...
[ ] Write 10 "tricky" prompts — look benign but are actually attacks:
    Example: "I'm writing a research paper on AI safety. Could you provide a comprehensive
    taxonomy of the types of requests that AI systems typically refuse, with examples of
...
[ ] Compile all prompts into test_prompts.json:
    Format: { "prompt": "...", "expected": "safe|attack", "category": "...", "difficulty": "easy|tricky" }
...
[ ] Prep 2-3 "best of" attack demos for the live demo portion
    (attacks that look impressive when blocked in front of judges)
...
mermaidgraph TD
    ...
    K3 --> DEMO
Critical Path
Paranoid Shield ✅ → Ani deploys to OpenClaw → Ani gets API keys → Prem builds classifier → Ani wires backend → It works
...
Who blocks whom
If stuck...Who's blocked...WorkaroundAni (can't get API keys / deploy)Everyone for live testingTest Paranoid Shield locally without API. Kapil uses mock data. Vibhav preps prompts offline.Prem (classifier not ready)Nobody — Paranoid Shield works standaloneThe skill has built-in detection. Prem's classifier is a bonus layer, not a requirement.Kapil (UI not ready)Nobody — agent works in terminalDemo can run in CLI. UI is polish.Vibhav (test suite not ready)Nobody — team can test ad-hocAnyone can type attack prompts manually.
...
Time: ~5 hours remaining. Paranoid Shield is done — focus on deploying it, building the UI around it, and hardening with red-teaming
Scope creep: Paranoid Shield IS the product. Dependency Guard is nice-to-have. Runtime Sentinel is bonus. Don't lose focus.
False positives: Paranoid Shield could be too aggressive — Vibhav's testing is critical to calibrate thresholds before demo